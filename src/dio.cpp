...
static void GetSpectrumForEstimation(const double *x, int x_length,
  ...
  // Complex spectrum multiplication in place (y_spectrum *= filter_spectrum)
  // -- K1 dispatch.
  world::simd::SpectrumMultiplyHalf(y_spectrum, filter_spectrum,
                                    y_spectrum, fft_size / 2 + 1);
...
static void GetFilteredSignal(int half_average_length, int fft_size,
  ...
  // Convolution only on [0..N/2] (K1 dispatch). Mirror copy not needed since
  // c2r reads only indices 0..N/2.
...
static int ZeroCrossingEngine(const double *filtered_signal, int y_length,
  ...
  // negative_going_points retained as unused workspace for API compatibility.
